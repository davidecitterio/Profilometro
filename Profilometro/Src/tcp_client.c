...
static err_t tcp_client_connected(void *arg, struct tcp_pcb *tpcb, err_t err);
 err_t tcp_listen_accept_conn(void * arg, struct tcp_pcb * newpcb,err_t err);

/* Private functions ---------------------------------------------------------*/

/**
  * @brief  Start tcp send data process
...
void tcp_client_connect(void)
{
  ...
  if (client_pcb != NULL)
  {
    ...
    IP4_ADDR( &DestIPaddr,  DEST_IP_ADDR0,  DEST_IP_ADDR1,  DEST_IP_ADDR2,  DEST_IP_ADDR3 );
		...
		tcp_bind(client_pcb, &LocalIPaddr, LOCAL_PORT);

		err_t err;
		
    /* connect to destination address/port */
    err = tcp_connect(client_pcb,&DestIPaddr,DEST_PORT,tcp_client_connected);
...
static err_t tcp_client_connected(void *arg, struct tcp_pcb *tpcb, err_t err)
{
  ...
  if (err == ERR_OK)   
  {
    ...
    if (es != NULL)
    {
      ...
      /* allocate pbuf */
      es->p_tx = pbuf_alloc(PBUF_TRANSPORT, strlen(data) , PBUF_POOL);
         
      if (es->p_tx)
      {       
        /* copy data to pbuf */
        pbuf_take(es->p_tx, data, strlen(data));
...
/**
  * @brief  Start tcp listen to specific port
  * @param  data: data to send
  * @retval None
  */
void tcp_start_listen(void)
{
	...
	IP4_ADDR( &LocalIPaddr, LOCAL_IP_ADDR0, LOCAL_IP_ADDR1, LOCAL_IP_ADDR2, LOCAL_IP_ADDR3 );
	listen_pcb = tcp_new();
	tcp_bind(listen_pcb, &LocalIPaddr, LISTEN_INCOMING_PORT);
	if (tcp_listen(listen_pcb)){}
	else {HAL_GPIO_WritePin(RED_GPIO_Port, RED_Pin, GPIO_PIN_SET);}
	tcp_accept(listen_pcb, tcp_listen_accept_conn);
}
